/*****************************************************
derived from tracker4.c
reason: to add serial output to lcd indicator


derived from tracker3.c
reason: To change limit to 1.8A for 30W panel
...
#define boost_current   180    //1.800A
#define trickle_current 160    //1.6A
...
double settm,riset,daytime,sunrise_min,sunset_min;
...
eeprom signed int e_set_latit = 1901,e_set_longitude =7283,e_low_angle=450,e_high_angle=1350,e_time_interval=15,e_set_timezone=550;
...
double fnday(long y,long m,long d,double h)
            {
...
void rise_set( double day,double m,double y,double h,double latit,double longit,double tzone)
{
double d,lamda;
...
if(adc_fl)
        ...
        if (adc_count == 150)
        {
        adc_chargecurrent = adc3421_read()/4;
        mux1 =0;
        mux2 =1;
        mux3 =0;
        } 
...
void cal_angle(void)
{
double sensitivity,angle_rad;

sensitivity = span_adc - zero_adc;
angle_rad = asin(((double)adc_angle - (double)zero_adc) /sensitivity);
...
// added to communicate with lcd indicator
        print_analog(adc_battery);
        print_analog(adc_pvolt);
        print_analog(adc_chargecurrent);
        print_analog(OCR1A);
        print_analog(angle);
        print_time();
        print_date();
