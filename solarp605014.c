...
eeprom signed int e_set_latit = 2650,e_set_longitude =7380,e_low_angle=-400,e_high_angle=400,e_time_interval=5,e_set_timezone=550;
...
if (rx_counter==1)
    {
    if (rx_buffer[0] != (char)(modbus_id))  //modbus_id
...
{
//set_latit,set_longitude,low_angle,high_angle,time_interval,target_time,time_elap,set_timezone;

//holding register data(R/W)
mb_data[0]= day;
mb_data[1] =month;
mb_data[2] = year;
mb_data[3] = hour;
mb_data[4] =minute;
mb_data[5] = set_latit;
mb_data[6] = set_longitude;
mb_data[7] = time_interval;
mb_data[8] = set_timezone;
mb_data[9] = set_width;
mb_data[10] = set_distance;

//input data
mb_inputdata[0] = angle;
mb_inputdata[1] = target_angle;
mb_inputdata[2] = target_time/60;   //target time in hours
mb_inputdata[3] = target_time%60;   //target time in minutes
}
...
//    while(1);
    switch (item1)
    {
        ...
        case 13:set_width+=100;
                if (set_width > 9999) set_width = 9900; 
                break;
        case 14:set_width+=10;
                if (set_width > 9999) set_width = 9900; 
                break;
        case 15:set_distance+=100;
                if (set_distance > 9999) set_distance = 9900; 
                break;
        case 16:set_distance+=10;
                if (set_distance > 9999) set_distance = 9900; 
                break;
        case 17:modbus_id+=100;
                if (modbus_id > 240) modbus_id = 240; 
                break;
        case 18:modbus_id+=1;
                if (modbus_id > 240) modbus_id = 240; 
                break; 
        case 19:modbus_rate+=100;
                if (modbus_rate > 5) modbus_rate = 5; 
                break;
        case 20:modbus_rate+=1;
                if (modbus_rate > 5) modbus_rate = 5; 
                break; 
...
if (key3_fl && pgm_fl)
{
    ...
    switch (item1)
    {
        ...
        case 12: set_timezone-=25;
                ...
                break;        
        case 13:set_width-=100;
                if (set_width <1) set_width = 10; 
                break;
        case 14:set_width-=10;
                if (set_width <1) set_width = 10; 
                break;
        case 15:set_distance-=100;
                if (set_distance <1) set_distance = 10; 
                break;
        case 16:set_distance-=10;
                if (set_distance <1) set_distance = 10; 
                break;
        case 17:modbus_id-=100;
                if (modbus_id <1) modbus_id = 1; 
                break;
        case 18:modbus_id-=1;
                if (modbus_id <1) modbus_id = 1; 
                break; 
        case 19:modbus_rate-=100;
                if (modbus_rate <1) modbus_rate = 1; 
                break;
        case 20:modbus_rate-=1;
                if (modbus_rate <1) modbus_rate = 1; 
                break; 
...
if (key4_fl && pgm_fl)
{
    ...
    if (set ==8 && (item1 <17 || item1 >18)) item1 = 17;
    if (set ==9 && (item1 <20 || item1 >20)) item1 = 20;
...
if (key1_fl && pgm_fl)
{
    ...
    case 7: e_set_distance = set_distance;
            break; 
    case 8: e_modbus_id =modbus_id;
            break;
    case 9: e_modbus_rate = modbus_rate;
            break;
    }
...
//                if (hour <0 || hour >24) rtc_err=1;         //added for rtc error
   ...
   case 8:     lcd_clear();
                lcd_putsf("ModBus ID");
                modbus_id = e_modbus_id;
                lcd_gotoxy(0,1);
                lcd_putsf("    ");        
                display_modbus(modbus_id);
                lcd_putsf(" ");
                char_id = blink_data = (abs(modbus_id)/100)%100 ;
                blink_locx =5;
                blink_locy =1;
                blink_fl =1;                              
                set =8;
                ...
                break;
   case 9:     lcd_clear();
                lcd_putsf("Modbus Rate ");
                modbus_rate = e_modbus_rate;
                lcd_gotoxy(0,1);
                lcd_putsf("   ");        
                
                switch(modbus_rate)
                    {
                    case 1: lcd_putsf("  9600");
                            break;
                    case 2: lcd_putsf(" 19200");
                            break;
                    case 3: lcd_putsf(" 38400");
                            break;
                    case 4: lcd_putsf(" 57600");
                            break;
                    case 5: lcd_putsf("115200");
                            break;
                    default:lcd_putsf("  9600");
                            e_modbus_rate = modbus_rate =0;
                            break;                    
                    }
//                display_modbus(modbus_rate);
                lcd_putsf(" Baud");
                char_rate = blink_data = (abs(modbus_rate)/100)%100 ;
                blink_locx =7;
                blink_locy =1;
                blink_fl =0;                              
                set =9;
...
if (mode ==0 && !manual_fl)
        ...
        if (mode0_seqcnt>=0 && mode0_seqcnt<=25)                //display date and time
                { 
                lcd_gotoxy(0,0);
//                put_message(Gyrox);
//                lcd_putsf("|");
...
//                    lcd_putsf(" deg.");    
        ...
        if (mode0_seqcnt>=26 && mode0_seqcnt<=40)            //display sunrise and sunset time
                { 
                lcd_gotoxy(0,0);
                lcd_putsf("sunrise: ");
        ...
        if (mode0_seqcnt>=41 && mode0_seqcnt<=55)
...
//                display_modbus(modbus_rate);
           ...
           switch (item1)
                ...
                case 16:char_distance =  blink_data = abs(set_distance)%100;
                        ...
                        break; 
                        break; 
                case 17:char_id = blink_data = (abs(modbus_id)/100)%100 ;
                        blink_locx =5;
                        blink_locy =1;
                        break;
                case 18:char_id =  blink_data = abs(modbus_id)%100;
                        blink_locx = 7;
                        blink_locy =1;
                        break;                         
                        break; 
                case 19:char_rate = blink_data = (abs(modbus_rate)/100)%100 ;
                        blink_locx =5;
                        blink_locy =1;
                        break;
                case 20:char_rate =  blink_data = abs(modbus_rate)%100;
                        blink_locx = 7;
                        blink_locy =1;
                        break;                 }       
        }
...
// added to hardcode limit from 50 to 125 degrees for ravindra energy
       if (target_angle > 600) target_angle = 600;
       if (target_angle < -600) target_angle = -600;
...
//#asm("wdr")
time_cnt1++;
//if(time_cnt1>=10)   //20 seconds record printout
//    {         
//    time_cnt1=0;
//    print_fl=1;
//    }
...
//UBRRH=0x00;    //9600 for 11.0592 
//UBRRL=0x47;



//UBRRH=0x00;        //9600 for 12mhz
//UBRRL=0x4D;



//UBRRH=0x00;    //9600  for 11.0592 baud rate
//UBRRL=0x47;


//UBRRH=0x00;    //19200  for 11.0592 baud rate
//UBRRL=0x23;

//UBRRH=0x00;     //38400 for 11.0592 baud rate
//UBRRL=0x11;

//UBRRH=0x00;     //57600  for 11.0592 baud rate
//UBRRL=0x0B;

UBRRH=0x00;      //115200 for 11.0592 baud rate
UBRRL=0x05;
