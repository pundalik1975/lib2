...
char blink_data;
signed int set_latit,set_longitude,low_angle,high_angle,time_interval,target_time,time_elap,set_timezone,ground_inclination,panel_height,panel_width;
//int ircommand;
...
eeprom signed int e_set_latit = 2650,e_set_longitude =7380,e_low_angle=500,e_high_angle=1250,e_time_interval=15,e_set_timezone=550,e_ground_inclination=0,e_panel_height=1,e_panel_width=1;
...
pitchacc =900.0 + 1800.0 * atan ((long)y_angle/sqrt((long)x_angle*(long)x_angle + (long)z_angle*(long)z_angle))/3.141592;
...
if (mode ==0 && !manual_fl)
        ...
        if (mode0_seqcnt>=0 && mode0_seqcnt<=65)                //display date and time
                { 
                lcd_gotoxy(0,0);
//                put_message(Gyrox);
//                lcd_putsf("|");
...
//        if (target_angle > 1250) target_angle = 1250;
//        if (target_angle < 500) target_angle = 500;
...
void backtrack(void)
{
//alpha: ground inclination(user setting);
//beta: sun angle(target angle);
//gamma sun and panel angle;
// b: factor to determine backtracking neccesary or not.
// w: width of panel(user setting); 
// d: distance between panels(user setting);


float alpha,beta,gamma,theta;
int b,d,w;
//change degrees to rads and 90 degrees offset 
alpha=dtor(ground_inclination);
beta = dtor(target_angle-900);
d= panel_height;
w= panel_width;

b = d * sin(beta - alpha) / w;
gamma = asin(b);
...
//#asm("wdr")
time_cnt1++;
if(time_cnt1>=30)   //half minute record printout
    {         
    time_cnt1=0;
    print_fl=1;
    }
...
/////////////////////////////////////////////////////////////////////                        
 ...
 //               adc_buffer = adc3421_read();
                ...
                relay2=1;
                timeout_cnt++;
                if(timeout_cnt >10000)      //once every 30 seconds
                        {
                        timeout_cnt =0;
