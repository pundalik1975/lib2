...
/*****************************************************
DERIVED FROM SOLARP60509.C
reason: GPS values read 
todo: check if gps is detected and if yes, update time,latitude and longitude.
if possible, read no. of satellites and display

Time_buffer[0] and [1] contain gps hours
[3] and [4] minutes
[6] and [7] seconds
...
eeprom long int e_zero_adc =0,e_span_adc=900;
...
short int gps_hour,gps_min,gps_sec,gps_fix;
int gps_longitude,gps_latitude;
short int gps_sat;
    ...
    void convert_time_to_IST()
{
    ...
    hour = hour+5;               //added 5:30 to time
...
void get_latitude(long lat_pointer)
    ...
    buflat =atof(degrees_buffer);
    gps_latitude = (int)(buflat * 100.0);
    #asm("sei");
    }
...
void get_longitude(int long_pointer)
    ...
    buflong =atof(degrees_buffer);
    gps_longitude = (int)(buflong * 100.0);
    #asm("sei");
    }
...
void get_gps(void)
{
        ...
        memset(degrees_buffer,0,degrees_buffer_size);
        get_altitude(GGA_Pointers[4]);
        x[0] = Altitude_Buffer[0];
        x[1] = Altitude_Buffer[1];
        gps_sat = atoi(x);
        get_altitude(GGA_Pointers[5]); 
        gps_fix = atoi(&Altitude_Buffer[0]);        
...
//                display_time2(settm*100);    
            ...
            lcd_gotoxy(0,0);
        lcd_putsf("Time: ");
        put_message2(gps_hour);  
        lcd_putchar(':');
        put_message2(gps_min);
        lcd_putchar(':');
        put_message2(gps_sec); 

        lcd_gotoxy(0,1);
        lcd_putsf("Lat: ");
        display_analog(gps_latitude);    /* display latitude in degree */
        lcd_gotoxy(0,2);
        
        lcd_putsf("Long: ");
        display_analog(gps_longitude);    
        lcd_gotoxy(0,3);
         lcd_putsf("Sat:");
        put_message2(gps_sat);
        lcd_putsf("  fix:");  
        put_message2(gps_fix);
